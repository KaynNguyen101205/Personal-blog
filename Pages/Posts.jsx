...
export default function Posts() {
  ...
  useEffect(() => {
    ...
    if (tagParam) {
      ...
      // Clear tag filter if not in URL
      setSelectedTag(null);
    }
    ...
    if (savedQuery) {
      ...
      // Clear search if not in localStorage
      setSearchQuery("");
    }
  ...
  const filteredPosts = posts.filter(post => {
    ...
    // Filter logic: "all" shows all posts (for admin) or all published (for non-admin)
    // "draft" shows only drafts (admin only)
    let matchesFilter = false;
    if (filterPublished === "all") {
      matchesFilter = true; // Show all posts that passed the admin check above
    } else if (filterPublished === "draft") {
      matchesFilter = !post.published; // Only drafts
